		...
		if($v!="") {
			$sql = "
				SELECT 
					...
					inputdt, k.nomorkontrak, vendor, k.uraian uraiank, tglawal, tglakhir, nilaikontrak kontrak, bayar, k.kid, k.signed, k.file_path, date(k.tgltagih) as tgltagih, isrutin,
					(CASE WHEN ((ka.nmrkontrak IS NULL) OR (ka.nmrkontrak IS NOT NULL and (signlevel = 1 and actiontype = 0) OR (signlevel = 4 and actiontype = 1))) AND (nilaikontrak - IFNULL(bayar, 0) > 0) THEN 0 ELSE 1 END) kontrakapproved, ka.*, kapel.signdt as app_pel, kaang.signdt as app_ang, kakeu.signdt as app_keu
				FROM notadinas n
...
*/
				...
				$hasilk .= "
					<tr>
						...
						<td>$row[vendor]</td>
						<td>$row[uraiank]</td>
						<td>".(empty($row["nomorkontrak"]) ? "" : ($row["isrutin"] == "" ? "-" : ($row["isrutin"] == 0 ? "NON RUTIN" : 'RUTIN')))."</td>
