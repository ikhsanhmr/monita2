...
if($_SESSION['roleid']<=3 || $_SESSION['roleid'] == 20) {
	...
	$sql = "SELECT 	COUNT(k.nomorkontrak) jumlah 
			FROM 	( 
					...
					kontrak k ON TRIM(k.nomorkontrak) = TRIM(ka.nmrkontrak) INNER JOIN
					notadinas_detail d ON k.nomorskkoi = d.noskk AND k.pos = d.pos1
			Where " . ( $_SESSION['roleid'] > 1 ? "((pelaksana <= 5 and (d.pos1 IN (Select akses From akses_pos Where nip = '".$_SESSION["cnip"]."'))) OR (pelaksana > 5 and (pelaksana in (Select akses From akses_bidang Where nip = '".$_SESSION['cnip']."')))) AND" : "" ) . " (signlevel = 2 AND actiontype = 1)";
	//echo "$sql<br>";
	$result = mysql_query($sql) or die (mysql_error());
	while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
		$ib = $row["jumlah"];
	...
	$sql = "SELECT 	COUNT(k.nomorkontrak) jumlah 
			FROM 	(
					...
					notadinas_detail d ON k.nomorskkoi = d.noskk AND k.pos = d.pos1 
			Where ".($_SESSION['cnip'] == '7602006A' ? "" : " ((pelaksana <= 5 and (d.pos1 IN (Select akses From akses_pos Where nip = '".$_SESSION["cnip"]."'))) or (pelaksana > 5 $sisip)) AND " )." (signlevel = 3 AND actiontype = 1)";
	//echo "$sql<br>";
	$result = mysql_query($sql) or die (mysql_error());
	while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
		$ib = $row["jumlah"];
...
$arr_exclude_dashboard = array(04, 05, 07, 13, 16);

if (!empty($v) || in_array($_SESSION['roleid'], $arr_exclude_dashboard) == false){
