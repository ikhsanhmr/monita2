...
//	echo ($edit!=""? "isi" : "kosong");
	...
	$cekNilaiPagu = mysql_query("
		SELECT 	d.*, kontrak, namapos 
...
*/
					...
					if($total_nilai_kontrak > $sisa_pagu){
						...
						echo '<script>alert("Penyimpanan gagal. Nilai kontrak #'.$kontrak.' lebih besar dari sisa pagu.");</script>';
						break;
					}

					$sql = ($edit==""? 
					"INSERT INTO kontrak(nomorskkoi, pos, nomorkontrak, uraian, vendor, tglawal, tglakhir, nilaikontrak, inputdt, pettycash,nodokumen,tgltagih,file_path,no_rab,inputby)
						VALUES ('$skk', '$pos', '$kontrak', '$uraian', '$vendor', '$awal', '$akhir', '$nilai', sysdate(), '$pc','$nodokumen','$tgltagih','$target_file','$nrab','$user')" :
