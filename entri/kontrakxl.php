...
*/
		...
		$sql = "
			SELECT	skkoi, n.nomornota, nipuser, pelaksana, b.namaunit, pos1, nilai1, namapos,	skk, DATE_FORMAT(tglskk, '%d-%m-%Y') as tglskk, pdp.jtmrp, 
					nilaidisburse, nomorscore, DATE_FORMAT(inputdt, '%d-%m-%Y %H:%i:%s') as inputdt, k.nomorkontrak, vendor, k.uraian uraiank, pdp.gdaset, 
					DATE_FORMAT(tglawal, '%d-%m-%Y') as tglawal, DATE_FORMAT(tglakhir, '%d-%m-%Y') as tglakhir, nilaikontrak, bayar, nomorprk, pdp.jtmaset, 
					pdp.gdrp, pdp.jtraset, pdp.jtrrp, pdp.sl1aset, pdp.sl1rp, pdp.sl3aset, pdp.sl3rp, pdp.keypointaset, kt.nama as name_type,
					pdp.keypointrp, pdp.file_path as pdp_file_path, DATE_FORMAT(date(k.tgltagih), '%m-%Y') as tgltagih
			...
			$parm
			ORDER BY skk, LPAD(pelaksana, 2, '0'), k.pos, k.tgltagih, nomorkontrak
		";
