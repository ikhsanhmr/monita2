...
<body>
		...
		switch ($userlvl) {
		    ...
		    case 3:
		    	...
		    	*/
				...
				$parm .= " AND ((signlevel IN (1, 2) AND actiontype = 1)) "; //case 1 dan 3
					...
					(
						...
						FROM		kontrak_approval t INNER JOIN 
										(
											...
											Where signlevel IN (1, 2) and actiontype = 1
				...
				FROM 	(
							...
							FROM	kontrak_approval t INNER JOIN 
									(
										SELECT nomorkontrak, MAX( id ) AS lastid
										FROM kontrak_approval
										WHERE year(signdt) IN (".(date("Y")-1).",".date("Y").")
										GROUP BY nomorkontrak
									)tm ON t.nomorkontrak = tm.nomorkontrak AND t.id = tm.lastid
						) ka INNER JOIN 
			...
			while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
				...
				$body .= "
					<tr data-verifikasi='1'>
						...
						<td>
							$row[nomorkontrak]
							<input type='hidden' name='k$no' id='k$no' value='".$row["nomorkontrak"]."'/>
						</td>
						<td>$row[namaunit]</td>
						<td>$row[vendor]</td>
						<td>$row[uraian]</td>
						<td>$row[namapos] <br /> ($row[pos1])</td>
						".($userlvl == 1 ? "<td><input type='text' name='doc$no' id='doc$no' value='".$row["nodokumen"]."'/></td>" : "<td>$row[nodokumen]</td>" )."
						<td align='right'>".number_format($row["nilaikontrak"])."</td>
						<td align='right'>".number_format($row["bayar"])."
							<input type='hidden' id='s$no' name='s$no' value='".$sisa."'/>
						</td>
						<td align='right'>".number_format($sisa)."</td>
						<td align='right'>
							".($userlvl != 2 ? "<input type='text' class='tagihan' name='t$no' id='t$no' value='".$nilaitagihan."'/>" : number_format($nilaitagihan)." <input type='hidden' name='t$no' id='t$no' value='".$nilaitagihan."'/>" )."
						</td>
						".($userlvl == 4 ? "<td><input required size='22' type='" . ($nice? "date": "text") . "' name='tglbyr$no' id='tglbyr$no' " . ($nice? "": "onChange='dateCheck(\"tglbyr$no\")' ") . " value='".date("Y-m-d")."' /></td>" : "" )."
						".($_SESSION["cnip"] == '6793235Z' || $_SESSION["cnip"] == '7602006A' ? "<td>$row[namauser]</td>" : "" )."
						<td>
							".($userlvl == 1 ? "<input type='text' name='ctt$no' id='ctt$no' value='".$row["catatan"]."'/>" : "$row[catatan]<input type='hidden' name='ctt$no' id='ctt$no' value='".$row["catatan"]."'/>" )."
						</td>
				...
				$sql = "
					...
					FROM	(
								...
								FROM	(
											...
											FROM 	(
														SELECT	t.nomorkontrak nmrkontrak, signed, signdt, 
																signlevel, actiontype, nilaitagihan, catatan, 
																catatanreject
														FROM	kontrak_approval t INNER JOIN 
																(
																	SELECT nomorkontrak, MAX( id ) AS lastid
											...
											WHERE 	COALESCE(nomorskkoi, '') != '' AND NOT nomorkontrak IS NULL AND ((signlevel IN (1, 2) AND actiontype = 1))
				...
				<form>
					...
					<table id='dataTables' class='display' cellspacing='0' width='100%'>
						<thead>
							...
							<th>Sisa</th>
							<th>Nilai Tagihan</th>
							".($userlvl == 4 ? "<th>Tgl Bayar (MM/DD/YYYY)</th>" : "" )."
					...
					<button type='button' class='btnsubmit' data-flag='0'>Reject</button>
					<button type='button' class='btnsubmit' data-flag='1'>Approve</button>
	...
	$(document).ready(function() {
		...
		$('.tagihan').on('keypress', function(evt){
		...
		$('.btnsubmit').on('click', function(){
			...
			if (rows_selected.length < 1){
				...
				$(".loading").hide();
				alert("Tidak ada tagihan yang dipilih.", "Warning");
			...
			if(actiontype == 0 && userlvl != 1){

				reason = prompt("Silahkan masukan alasan kontrak ditolak:", "");
			...
			$.each(rows_selected, function(index, data){
				...
				var tgh = $("#t" + no).val();
				var ctt = $("#ctt" + no).val();
				var tglbyr = $("#tglbyr" + no).val();

				data_terpilih.push([nok, dok, sisa, tgh, ctt, tglbyr]);
			});
