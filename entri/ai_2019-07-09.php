...
//		$parm .= ($p2==""? "": " and SUBSTR(tanggalskki, 1, 7) <= '$p2'");
		...
		if ($user == "93162829ZY"){
			...
			$parm .= ($b0==""? " AND c.pos1 IN (Select akses From akses_pos Where nip = '$user') ": " and (nipuser = '$b0' or pelaksana = '$b0' or c.pos1 IN (Select akses From akses_pos Where nip = '$user'))");

		}else{

			// $parm .= ($b0==""? "": " and (g.id = '$b0' or pelaksana = '$b0')");
			$parm .= ($b0==""? "": " and (nipuser = '$b0' or pelaksana = '$b0')");
		}
		...
		if($v!="") {
			$sql = "
				SELECT 	n.nomornota, nipuser, pelaksana, b.namaunit, pos1, nilai1, namapos, nomorwbs, nomorprk, nomorscore,		
						...
						nilaiwbs wbs, inputdt, k.nomorkontrak, k.nodokumen nodokumen, vendor, k.uraian uraiank, tglawal, tglakhir,
						nilaikontrak kontrak, k.signed, bayar, k.kid , k.file_path, ka.*, rb.no_rab, rb.nilai_rp, pdp.jtmaset, 
						pdp.jtmrp, pdp.gdaset, pdp.gdrp, pdp.jtraset, pdp.jtrrp, pdp.sl1aset, pdp.sl1rp, pdp.sl3aset, pdp.sl3rp, 
						pdp.keypointaset, pdp.keypointrp, s.jtm, s.gd, s.jtr, s.sl1, s.sl3, s.keypoint, s.nilaianggaranjtm, 
						s.nilaianggarangd, s.nilaianggaranjtr, s.nilaianggaransl1, s.nilaianggaransl3, s.nilaianggarankp, 
