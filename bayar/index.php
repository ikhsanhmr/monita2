...
<body>
	<?php
		...
		echo "
			...
			<a href='#' onClick='upload()'>(+) Upload Realisasi Bayar</a>		
		";
		
		if($v!="") {
			$sql = "
				SELECT 
					skkoi, n.nomornota nd, nip, nipuser, pelaksana, g.id nick, pos1, namapos, b.namaunit, skk, oi.uraian oiuraian, r.nodokrep nodokrep,
					nomorkontrak, k.pos kpos, k.uraian kuraian, vendor, tglawal, tglakhir, nilaikontrak, bayar,coalesce(ke,0) ke, SIGNED sgd
				FROM notadinas n
				...
				LEFT JOIN (SELECT nokontrak, nodokrep, SUM(nilaibayar) bayar, count(*) ke FROM realisasibayar GROUP BY nokontrak) r ON k.nomorkontrak = r.nokontrak
				WHERE n.progress >= 7 
				AND (BAYAR<=(0.95*NILAIKONTRAK)||BAYAR IS NULL )
				AND COALESCE(oi.skk, 0) != '' 
				AND NOT nomorkontrak IS NULL 
					/*AND bayar < nilaikontrak*/
					$parm
