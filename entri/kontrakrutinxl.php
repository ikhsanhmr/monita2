<?php
	...
	$parm .= ($p1==""? "": " and SUBSTR(inputdt, 1, 7) >= '$p1'");
	$parm .= ($p2==""? "": " and SUBSTR(inputdt, 1, 7) <= '$p2'");
	...
	if($v!="") {
		...
		$sql = "
			SELECT 	n.nomornota, nipuser, pelaksana, nomorwbs, s.uraian uraians, nilaianggaran anggaran, 
					DATE_FORMAT(tanggalskko, '%d-%m-%Y') as tanggalskko, k.uraian uraiank, k.kid, k.signed,
					DATE_FORMAT(inputdt, '%d-%m-%Y %H:%i:%s') as inputdt, nilaikontrak kontrak, isrutin, 
					DATE_FORMAT(tglawal, '%d-%m-%Y') as tglawal, DATE_FORMAT(tglakhir, '%d-%m-%Y') as tglakhir,
					DATE_FORMAT(date(k.tgltagih), '%m-%Y') as tgltagih, nomorcostcenter, nomorskko noskk, 
					DATE_FORMAT(kapel.signdt, '%d-%m-%Y %H:%i:%s') as app_pel, bayar, k.nomorkontrak, vendor, 
					DATE_FORMAT(kaang.signdt, '%d-%m-%Y %H:%i:%s') as app_ang, nilaiwbs wbs, k.file_path, 
					DATE_FORMAT(kakeu.signdt, '%d-%m-%Y %H:%i:%s') as app_keu, b.namaunit, pos1, namapos, 
					nilaidisburse disburse, nilai1
		...
		while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
			...
			$parm .= "
				<tr>
					...
					<td>$row[vendor]</td>
					<td>$row[uraiank]</td>
					<td>".($row["tgltagih"] != '00-0000' ? $row["tgltagih"] : '-')."</td>
					<td>$row[tglawal]</td>
					<td>$row[tglakhir]</td>
