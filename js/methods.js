function savepwd() {
	...
	document.getElementById("rpwd").style.borderColor = ((pwd!=rpwd)||(rpwd=="")? "red": "grey");
	if(pwd!=rpwd) {
	...
	var opwd = document.getElementById("opwd").value.trim();
	var parm = "nip=" + nip + "&opwd=" + opwd + "&pwd=" + pwd;
	
	
	
	document.getElementById("opwd").style.borderColor = (opwd==""? "red": "grey");
	cont = (opwd==""? false: true);	

	document.getElementById("pwd").style.borderColor = (pwd==""? "red": "grey");
	if(cont) {cont = (document.getElementById("pwd").value.trim()==""? false: true);}
	...
	if(cont) {
		...
		xmlhttp.onreadystatechange=function() {
			if (xmlhttp.readyState==4 && xmlhttp.status==200) {
				...
				window.open("pwd.php", "_self");
			}
		}
		
		xmlhttp.open("POST","pwdsave.php",true);
...
function myvalue(me) {
	...
	var td = document.getElementById("nonotadinas").value;
	//alert(td);
...
//			alert(document.getElementById("infopagu"+me).innerHTML);
//			var dummy = (isNaN(parseFloat(xmlhttp.responseText))? 0: parseFloat(xmlhttp.responseText));
			document.getElementById("pagu"+me).value=dummy[0]; // - mylimit(me);
			document.getElementById("pakai"+me).value=dummy[1]; // - mylimit(me);
			document.getElementById("sisa"+me).value=dummy[2]; // - mylimit(me);
			document.getElementById("infopagu"+me).innerHTML=dummy[3];//xmlhttp.responseText; // - mylimit(me);
//			alert("cek " + dummy);
//			nilai_usulan(me);
		}
	}
	xmlhttp.open("GET","myvalue.php?q="+mine+"&th="+th+"&td="+td,true);
	xmlhttp.send();
}
...
function all_limit() {
	...
	for(i=0; i<lfrm; i++) {
		switch(frm.elements[i].id.substr(0,3)) {
			case "pos":
				lim = false;
				for(k=0; k<j; k++) { if(frm.elements[i].value == mypos[k]) {lim = true; break;} }
				j = (lim? k: j+1);
...
*/
	...
	for(i=0; i<mypos.length; i++) {
		dummy = 0;
		for(j=0; j<lfrm; j++) {
			myid = frm.elements[j].id.substr(3, frm.elements[j].id.length-3);
			
			if((frm.elements[j].id.substr(0,3)=="pos") && (frm.elements[j].value==mypos[i])) {
				dummy += parseFloat(document.getElementById("nilai"+myid).value);
//				alert(mypos[i] + " -- " + dummy + " -- " + parseFloat(document.getElementById("sisa"+myid).value) + " -- " + (dummy > parseFloat(document.getElementById("sisa"+myid).value)));
				
				if(dummy > parseFloat(document.getElementById("sisa"+myid).value)) {
//					alert("X--" + mypos[i] + " -- " + dummy + " -- " + parseFloat(document.getElementById("sisa"+myid).value) + " -- " + (dummy > parseFloat(document.getElementById("sisa"+myid).value)));
					lim = false;
					document.getElementById("nilai"+myid).style.borderColor = "red";
				}
			}
		}
		/*
		alert(mypos[i] + " -- " + myval[i] + " -- " + mylim[i]);
		dummy = mypos[i];
...
function viewnilai(loc) {
	...
	var p = document.getElementById("pos").value;
	var url = encodeURI("nilaisemua.php?s=" + s + "&p=" + p);
...
// number format
function numberFormat(me, event) {
	var cpos = getCaretPosition(me);//+1;
	var x = event.which || event.keyCode;
	
	var dummy0 = me.value.length;
	var dummy1;
	
	if((x>=96 && x<=105)||(x>=48 && x<=57) || x==8 || x==46) {
		var dummy = "";
		var dummyval = "";
		var counter = 0;			
		
		for(var i=me.value.length; i>0; i--) {
			dummyval = me.value.substr(i-1,1);
			
			if(dummyval >= "0" && dummyval <= "9") {
				counter++;
		...
		//document.getElementById("txt1").value = dummy;
		me.value = dummy;
		dummy1 = me.value.length;
		setCaretPosition(me, cpos+(dummy1-dummy0));
	} 
}


function getCaretPosition(me) {
	...
	if (document.selection) {
		...
		Sel.moveStart ('character', -me.value.length);
 
		CaretPos = Sel.text.length;
	}
	// Firefox support
	else if (me.selectionStart || me.selectionStart == '0')
		CaretPos = me.selectionStart;
...
//			var el = document.getElementById(elemId);
	me.value = me.value;
	//var dummy = me.value.length;
	
	//alert(me.id + " " + me.value + " " + dummy);
...
function addmeaki() {
	...
	var dummy = 0;
	for(var i=0; i<x.length; i++) {
		if(x.elements[i].id.substr(0,3)=="aki") {
			var regex = new RegExp("\\,","g");
			var resaki = parseInt(x.elements[i].value.replace(regex,""));
			dummy += (isNaN(resaki)? 0: resaki);
			console.log(resaki);
		}
	}
	document.getElementById("jumlahaki").value = dummy;
}

function tambahaki(prd) { 
	
	var url = "subaki.php?prd=" + prd;
	window.open(url,"_self");
}

function hapuspaguaki(pos, prd, namaunit) {
//	alert(pos);

	if(confirm("Hapus SELURUH Data AKI " + namaunit + "?")) {
//		var url = "hapuspagu.php?prd=" + document.getElementById("tprd").value + "&pos=" + pos;
		var url = "hapuspagu.php?prd=" + prd + "&pos=" + pos;
		window.open(url,"_self");
	}
...
function updatetagihan(no) {
	
	var nilai = document.getElementById("nilai" + no).value;
	document.getElementById("tagihan" + no).value = nilai;
}