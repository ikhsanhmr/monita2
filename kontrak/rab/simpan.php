...
// ini saya buat di file form nya tida ada
$active = 1;

$message = "";

if ($edit != "") {
	$ceklimitrab = mysqli_query($mysqli, "select SUM(nilaikontrak) as totalkontrak from kontrak where no_rab = '$norab' group by no_rab");
	$query = mysqli_fetch_assoc($ceklimitrab);
...
if ($message == "") {
	...
	$qry = ($edit == "" ? "insert into rab(skk, no_rab,nilai_rp,tgl_rab,uraian_kegiatan,nip,status) values('$skk','$norab','$nilai','$tanggalrab','$uraian','$nip',$active)" : "update rab set 
									skk = '$skk',
									nilai_rp = '$nilai', 
									tgl_rab = '$tanggalrab',
	...
	$sukses = mysqli_query($mysqli, $qry);
