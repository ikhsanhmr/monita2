		...
		if($v!="") {
			$sql = "
				SELECT 
					...
					nomorkontrak, k.pos kpos, k.uraian kuraian, vendor, tglawal, tglakhir, nilaikontrak, bayar, SIGNED sgd
				FROM notadinas n
				...
				WHERE n.progress=7
					...
					$parm
				ORDER BY nd, skk, pos1, nomorkontrak limit 100";
			//echo $parm;
			//echo $sql;
			...
			while ($row = mysqli_fetch_array($result)) {
				...
				$parm .= "
					<tr>
						...
						($row["skkoi"]=="SKKI"? 
							($row["sgd"]==""? "<a href='#' onClick='edit(\"$row[nomorkontrak]\")'>Edit</a><br><a href='#' onClick='hapus(\"$row[nomorkontrak]\")'>Hapus</a>": "<strong>SIGNED</strong>"): 
							($row["bayar"]==""? "<a href='#' onClick='edit(\"$row[nomorkontrak]\")'>Edit</a><br><a href='#' onClick='hapus(\"$row[nomorkontrak]\")'>Hapus</a>": "")
						)
						.
						"</td>
					</tr>";
