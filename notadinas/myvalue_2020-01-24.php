...
//		SELECT COALESCE(rppos,0)-COALESCE(nilai,0) rppos FROM 
	$query = "
		SELECT saldo.*, posx, nilaix, COALESCE(rppos,0)-COALESCE(nilaix,0) sisax FROM (
			SELECT tahun, kdsubpos, rppos FROM saldopos WHERE tahun = $th
			UNION 
			SELECT tahun, kdsubpos, rppos FROM saldopos2 WHERE tahun = $th
			UNION
			SELECT tahun, kdsubpos, rppos FROM saldopos3 WHERE tahun = $th
			UNION
			SELECT tahun, kdsubpos, rppos FROM saldopos4 WHERE tahun = $th
		) saldo
		LEFT JOIN (
			SELECT pos1 posx, SUM(COALESCE(nilai1,0)) nilaix 
			FROM notadinas n 
			LEFT JOIN notadinas_detail d ON n.nomornota = d.nomornota
			WHERE YEAR(tanggal) = $th AND d.progress >= 7 AND n.nomornota != '$td'
			GROUP BY pos1
