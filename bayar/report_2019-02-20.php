...
//		$parm .= ($p2==""? "": " and SUBSTR(tglbayar, 1, 7) <= '$p2'"); //
		...
		if($v!="") {
			...
			if(isset($_REQUEST["e"])) {
				...
				$sql = (
					$_REQUEST["e"]=="1"? 
					"update realisasibayar set nilaibayar = '$_REQUEST[r]', tglbayar = '$_REQUEST[d]' where bayarid = '" . substr($_REQUEST["n"],1,strlen($_REQUEST["n"])-1) . "'": 
			...
			$sql = "
				SELECT 
					n.skkoi skkoi, nipuser, g.id iduser, pelaksana, b.namaunit, skk, pos1, namapos, k.nomorkontrak, k.uraian uraiank, k.tglawal tglmulai, k.tglakhir tglakhir, k.nilaikontrak nilaikontrak, k.vendor vendor, k.nodokumen nodokumen, nilaibayar, tglbayar, bayarid, aset.jtmaset jtma, aset.jtraset jtra, aset.gdaset gda, s.nomorprk prk, s.nomorscore score, s.nilaitunai nilaiskk, s.tanggalskki tanggalterbit, s.posinduk posinduk, unit.namaunit nmunit, r.pmn pmn, r.nodokrep, r.keterangan
				FROM (
					SELECT nomorskko skk FROM skkoterbit UNION 
					SELECT nomorskki skk FROM skkiterbit
				) o
				LEFT JOIN skkiterbit s ON o.skk = s.nomorskki 
			...
			while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
				...
				$parm .= "
					<tr>
						...
						<td>$row[nodokrep]</td>
						<td>$row[pmn]</td>
						<td>$row[tglbayar]<div id='t$no' style='display:none'><input type='date' name='d$no' id='d$no' value='$row[tglbayar]'></div></td>
						<td align='right'>" . number_format($row["nilaibayar"],0) . "<div id='n$no' style='display:none'><input type='number' name='v$no' id='v$no' value='$row[nilaibayar]'></div></td>
