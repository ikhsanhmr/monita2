...
<body>
	<?php
		...
		$sql = ($con==""? 
			...
			//-ASLI-"SELECT DISTINCT noskk FROM notadinas_detail d left join notadinas n on d.nomornota = n.nomornota LEFT JOIN kontrak k ON d.noskk = k.nomorskkoi WHERE d.progress = 7 AND k.nomorskkoi IS NULL" . ($org=="" || $rog=="1"? "": "AND (nipuser = '$nip' or pelaksana = '$org') ") . "ORDER BY noskk":
			
			"SELECT DISTINCT noskk FROM notadinas_detail d left join notadinas n on d.nomornota = n.nomornota LEFT JOIN kontrak k ON d.noskk = k.nomorskkoi WHERE d.progress = 7 AND k.nomorskkoi IS NULL || k.nomorskkoi IS NOT NULL" . ($org=="" || $org=="1"? "": "AND (nipuser = '$nip' or pelaksana = '$org') ") . "ORDER BY noskk":
		...
		echo "
			<h2>INPUT KONTRAK</h2>
			<form name='fkontrak' id='fkontrak' method='post' action='simpan.php' onsubmit='return validate()'>
				<table border='1'>
					<tr>
