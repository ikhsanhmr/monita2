...
*/
	$sql = "
SELECT nip, nipuser, pelaksana, namaunit, pos1, k.*, ke, sudahbayar 
FROM kontrak k 
	LEFT JOIN (SELECT nokontrak, COUNT(*) ke, SUM(nilaibayar) sudahbayar FROM realisasibayar GROUP BY nokontrak) b ON k.nomorkontrak = b.nokontrak 
	LEFT JOIN (SELECT nip, nipuser, pelaksana, namaunit, noskk, pos1 FROM notadinas n LEFT JOIN notadinas_detail d ON n.nomornota = d.nomornota LEFT JOIN bidang b ON d.pelaksana = b.id) nd 
	ON k.nomorskkoi = nd.noskk AND k.pos = nd.pos1	
WHERE (COALESCE(nilaikontrak,0) - COALESCE(sudahbayar,0) > 0) and TRIM(LOWER(nomorskkoi)) LIKE '%skk%" . ($role==4? "o": "i") . "%'ORDER BY nomorskkoi, nomorkontrak";
//	LEFT JOIN (SELECT nip, nipuser, pelaksana, noskk, pos1 FROM notadinas n LEFT JOIN notadinas_detail d ON n.nomornota = d.nomornota) nd ON k.nomorskkoi = nd.noskk AND k.pos = nd.pos1 
//	echo $sql;
