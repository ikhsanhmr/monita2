		    ...
		    case 3:
		    	...
		    	*/
		        ...
		        if($_SESSION["cnip"] != '6793235Z'){ // case 2
					...
					$parm .= " AND ( 
								(pelaksana <= 5 and (d.pos1 IN (Select akses From akses_pos Where nip = '".$_SESSION["cnip"]."'))) OR 
								(pelaksana > 5 and (pelaksana in (Select akses From akses_bidang Where nip = '".$_SESSION['cnip']."')))
							)";
		        }
		    ...
		    case 4:
		    	...
		    	*/
				...
				if($_SESSION["cnip"] != '7602006A'){ // case 2
					...
					$parm .= " and (
								(pelaksana <= 5 and (d.pos1 IN (Select akses From akses_pos Where nip = '".$_SESSION["cnip"]."')))
								or 
								(pelaksana > 5 $sisip)
							)";
		        }
		...
		*/
		...
		if($_SESSION["cnip"] == '7602006A'){
			$otherjoin .= " LEFT JOIN
						(
							...
							where 	roleid IN (4,5)
							GROUP BY akses
						) ap ON (pelaksana <= 5 and (d.pos1 = ap.akses)) OR (pelaksana > 5 and (pelaksana = ap.akses and d.pos1 NOT IN ('52.3.04','54.2.04')) or (d.pos1 = ap.akses and ap.akses IN ('52.3.04','54.2.04')))";
		...
		if($_SESSION["cnip"] == '6793235Z'){
			$otherjoin .= " LEFT JOIN
						(
							...
							where 	roleid IN (2,3)
							GROUP BY akses
						) ap ON (pelaksana <= 5 and (d.pos1 = ap.akses)) OR (pelaksana > 5 and (pelaksana = ap.akses))";
		}
		...
		echo ( $userlvl == 1 ? "<a href='#' onClick='upload()'>(+) Upload Bayar</a>" : "" );
			...
			if($_SESSION["cnip"] == '6793235Z'){
				...
				$sql = "
					SELECT	ap.nama, ap.akses, ap.namawewenang, COUNT(k.nomorkontrak) as jml, SUM(ka.nilaitagihan) as nilai,
							(CASE WHEN ap.grouping = 'WILSU' THEN CONCAT('WILSU - ', ap.namawewenang) ELSE ap.namawewenang END) as wewenang
					FROM 	(
								SELECT	t.nomorkontrak nmrkontrak, signed, signdt, signlevel, actiontype, 
										nilaitagihan, catatan, catatanreject
								FROM	kontrak_approval t INNER JOIN 
							...
							(
								SELECT 	akses, GROUP_CONCAT(nama SEPARATOR ', ') AS nama, b.namaunit AS namawewenang, 
										'NONWILSU' as grouping
								FROM 	akses_bidang ap INNER JOIN 
										user u On ap.nip = u.nip LEFT JOIN 
										bidang b ON ap.akses = b.id
								where 	roleid IN (2,3)
								GROUP BY akses
								UNION
								SELECT 	akses, GROUP_CONCAT(nama SEPARATOR ', ') AS nama, p.namapos AS namawewenang, 
										'WILSU' as grouping
								FROM 	akses_pos ap LEFT JOIN
										user u On ap.nip = u.nip LEFT JOIN 
										(
					...
					GROUP BY ap.grouping, ap.akses, ap.namawewenang, ap.nama
					ORDER BY ap.grouping DESC, wewenang ASC";
				//echo $sql;
			...
			if($_SESSION["cnip"] == '7602006A' || $_SESSION["cnip"] == '6793235Z'){
				...
				$sql = "
					SELECT	ap.nama, ap.akses, ap.namawewenang, SUM(CASE WHEN n.skkoi = 'SKKI' THEN 1 ELSE 0 END) as jmlskki, 
							...
							(CASE 	WHEN ap.grouping = 'WILSU' and pelaksana <= 5 THEN CONCAT('WILSU - ', ap.namawewenang) 
									WHEN ap.grouping = 'WILSU' and pelaksana > 5 THEN CONCAT(b.namaunit, ' - ', ap.namawewenang) 
									ELSE ap.namawewenang 
							END) as wewenang
					FROM 	(
								SELECT	t.nomorkontrak nmrkontrak, signed, signdt, signlevel, actiontype, 
										nilaitagihan, catatan, catatanreject
							...
							(
								...
								GROUP BY akses
									...
									(pelaksana > 5 and ((pelaksana = ap.akses and d.pos1 NOT IN ('52.3.04','54.2.04')) or (d.pos1 = ap.akses and ap.akses IN ('52.3.04','54.2.04'))))
