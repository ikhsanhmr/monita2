		...
		if($v!="") {
			...
			$sql = "
				SELECT 	n.nomornota, nipuser, pelaksana, b.namaunit, pos1, nilai1, namapos, nomorwbs, nomorcostcenter, nomorskko noskk, s.uraian uraians, 
						nilaianggaran anggaran, nilaidisburse disburse, nilaiwbs wbs, k.file_path, bayar, k.nomorkontrak, vendor, k.uraian uraiank, k.kid, 
						k.signed, nilaikontrak kontrak, isrutin, DATE_FORMAT(tanggalskko, '%d-%m-%Y') as tanggalskko, 
						DATE_FORMAT(inputdt, '%d-%m-%Y %H:%i:%s') as inputdt, DATE_FORMAT(tglawal, '%d-%m-%Y') as tglawal, 
						DATE_FORMAT(tglakhir, '%d-%m-%Y') as tglakhir, DATE_FORMAT(date(k.tgltagih), '%m-%Y') as tgltagih, 
						DATE_FORMAT(kapel.signdt, '%d-%m-%Y %H:%i:%s') as app_pel, DATE_FORMAT(kaang.signdt, '%d-%m-%Y %H:%i:%s') as app_ang, 
