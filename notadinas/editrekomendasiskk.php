...
$result = mysqli_query($mysqli, $sql) or die('Unable to execute query. ' . mysqli_error($mysqli)) or die(mysqli_error($mysqli));
while ($row = mysqli_fetch_array($result)) {
	$tgl = $row["tgl"];
}
mysqli_free_result($result);
...
FROM notadinas n LEFT JOIN notadinas_detail d ON n.nomornota = d.nomornota
LEFT JOIN (
	SELECT kdsubpos, rppos, tahun FROM saldopos
	UNION
	SELECT kdsubpos, rppos, tahun FROM saldopos2
	UNION
	SELECT kdsubpos, rppos, tahun FROM saldopos3
	UNION
	SELECT kdsubpos, rppos, tahun FROM saldopos4
) s ON YEAR(tanggal) = tahun AND pos1 = kdsubpos
LEFT JOIN (
	...
	<form name='tambahrekomendasi' method=POST id='tambahrekomendasi' action="proseseditrekomendasi.php" autocomplete="off" onsubmit="return ndvalidate(1)">
		...
		<table>
			<tr>
				...
				<td>
					<input name="newnonotadinas" id="newnonotadinas" maxlength="64" size="50" type="text" value="<?php echo $nomornota; ?>" />
					<input name="nonotadinas" id="nonotadinas" maxlength="64" size="50" type="hidden" value="<?php echo $nomornota; ?>" />
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td>Jenis</td>
				<td><input name="jenis" id="jenis" maxlength="64" size="50" type="text" value="<?php echo $skkoi; ?>" readonly /></td>
			</tr>
			...
			<tr>
				<td>Nilai Usulan</td>
				<td>
					<input name="nilairekom" id="nilairekom" maxlength="64" size="50" type="text" onchange="formatme(this)" value="<?php echo $nilaiusulan; ?>" readonly />
				</td>
			</tr>
			<tr>
				<td>
				...
				<td>
					<div id="mydiv"><?php echo $mydiv1 ?></div>
				</td>
			</tr>
			<tr>
				<td colspan="2">
