...
<body>
	...
	if (isset($_REQUEST['prd'])) {
		...
		$sql = "SELECT p.*, rppos, rpaki FROM posinduk" . ($_REQUEST["ke"] + 1) . " p LEFT JOIN saldopos" . ($_REQUEST["ke"] + 1) .
		...
		while ($row = mysqli_fetch_array($result)) {
			...
			echo "
				...
				"' onchange=\"formatme(this); nilaisubpagu(this, '$tot')\">
					...
					<td align='right'>
						<input class='right' type='hidden' name='b$dummy' id='a$dummy' value='" .
				(number_format($row['rpaki']) == 0 ? "" : number_format($row['rpaki'])) . "'>
						<input class='right' type='text' name='b$dummy' id='a$dummy' value='" .
				(number_format($row['rpaki']) == 0 ? "" : number_format($row['rpaki'])) .
				"' onchange=\"formatme(this); nilaisubpagu(this, '$tot')\">
					</td>
		...
		echo "
			...
			($_REQUEST["ke"] == 1 ?
				...
				"subpos.php?prd=$_REQUEST[prd]&pos=$back&ke=" . ($_REQUEST["ke"] - 1)) . "', '_self')\">
