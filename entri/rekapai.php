...
<head>
	...
	<script type="text/javascript">
		function viewk(x) {
			...
			var url = encodeURI((x==undefined? "rekapai.php": "rekapaiexcel.php") + "?p1="+p1+"&p2="+p2+"&b="+b+"&p="+p+"&k="+k+"&kpos="+kpos+"&v=1");
			//alert(url);
...
//		$parm .= ($p2==""? "": " and SUBSTR(tanggalskki, 1, 7) <= '$p2'");
		...
		//echo "parm : $parm<br>";
		echo "
			<h2>Rekap Monitoring Penyerapan Anggaran Investasi</h2>
			<table>
				<tr>
		...
		if($v!="") {
			$sql = "
				SELECT 
					...
					nomorwbs, nomorprk, nomorscore, nomorskki noskk, s.uraian uraians, DATE_FORMAT(tanggalskki, '%d-%m-%Y') as tanggalskki, DATEDIFF(SYSDATE(), tanggalskki) umur, nilaianggaran anggaran, nilaidisburse disburse, nilaiwbs wbs, 
				...
				$parm
				ORDER BY nomorskki, LPAD(pelaksana, 2, '0'), pos1";
			//echo $sql;
