		...
		echo ( $userlvl == 1 ? "<a href='#' onClick='upload()'>(+) Upload Bayar</a>" : "" );
			...
			$sql = "
				SELECT	nomorskkoi, pos1, nomorkontrak, b.namaunit, vendor, k.uraian uraian, 
						...
						ka.nilaitagihan, ka.catatan, ka.catatanreject, namapos, k.inputdt, kapel.signdt as app_pel, 
						kaang.signdt as app_ang
				FROM 	(
						...
						) kaang  ON TRIM(k.nomorkontrak) = TRIM(kaang.nmrkontrak)
			...
			if ($_SESSION["roleid"] == 1 || $_SESSION["roleid"] == 21){
				...
				$sql = "
					...
					FROM	(
								SELECT	nomorskkoi, pos1, nomorkontrak, b.namaunit, vendor, k.uraian uraian, 
										nodokumen, nilaikontrak, bayar, ka.signed, ka.signdt, ka.signlevel, ka.actiontype, 
										ka.nilaitagihan, ka.catatan, ka.catatanreject, namapos, k.inputdt, 
										kapel.signdt as app_pel, kaang.signdt as app_ang, ap.nama namauser
								FROM 	(
											SELECT	t.nomorkontrak nmrkontrak, signed, signdt, signlevel, actiontype, 
													nilaitagihan, catatan, catatanreject
											FROM	kontrak_approval t INNER JOIN 
											...
											FROM 	(
													...
													(
														...
														where 	roleid IN (2,3)
															...
															(pelaksana > 5 and (pelaksana = ap.akses or (d.pos1 = ap.akses and ap.is_all_unit = 1)))
			...
			echo "
				<form name='frm' id='frm' method='post' action='simpan.php'>
					<input type='hidden' name='level' value='".($userlvl == 1 && $org <= 5 ? 2 : $userlvl)."' />
