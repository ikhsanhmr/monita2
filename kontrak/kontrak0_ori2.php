...
<body>
	<?php
		...
		$sql = ($con==""? 
			...
			"SELECT DISTINCT noskk FROM notadinas_detail d left join notadinas n on d.nomornota = n.nomornota LEFT JOIN kontrak k ON d.noskk = k.nomorskkoi WHERE d.progress in(7,9)   " . ($org=="" || $rog=="1"? "": "AND (nipuser = '$nip' or pelaksana = '$org') ") . "ORDER BY noskk":
		...
		echo "
			<h2>INPUT KONTRAK</h2>
			<form name='fkontrak' id='fkontrak' method='post' action='simpan.php' onsubmit='return validate()' enctype='multipart/form-data'>
				<table border='1'>
					<tr>
