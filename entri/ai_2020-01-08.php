...
//		$parm .= ($p2==""? "": " and SUBSTR(tanggalskki, 1, 7) <= '$p2'");
		...
		if($v!="") {
			...
			// 			nomorscore,	nomorskki noskk, s.uraian uraians, s.nilaianggaransl1, s.nilaianggarankp, 
			// 			DATE_FORMAT(tanggalskki, '%d-%m-%Y') as tanggalskki, nilaianggaran anggaran, s.jtm, ka.*, 
			// 			nilaidisburse disburse, nilaiwbs wbs, pdp.file_path as pdp_file_path, s.sl1, rb.no_rab, 
			// 			DATE_FORMAT(inputdt, '%d-%m-%Y %H:%i:%s') as inputdt, k.nomorkontrak, vendor, k.kid, 
			// 			s.sl3, k.nodokumen nodokumen, k.uraian uraiank, s.nilaianggaranjtm, s.nilaianggarangd, 
			// 			DATE_FORMAT(tglawal, '%d-%m-%Y') as tglawal, k.file_path, nilaikontrak kontrak, pdp.sl1rp,
			// 			DATE_FORMAT(tglakhir, '%d-%m-%Y') as tglakhir, k.signed, bayar, rb.nilai_rp, pdp.jtmaset, 
			// 			pdp.jtmrp, pdp.gdaset, pdp.gdrp, pdp.jtraset, pdp.jtrrp, s.gd, pdp.sl1aset, s.keypoint, 
			// 			pdp.sl3aset, pdp.sl3rp, pdp.keypointaset, pdp.keypointrp, s.jtr, s.nilaianggaranjtr, 
			// 			DATE_FORMAT(kaang.signdt, '%d-%m-%Y %H:%i:%s') as app_ang, s.nilaianggaransl3, 
			// 			DATE_FORMAT(kakeu.signdt, '%d-%m-%Y %H:%i:%s') as app_keu, 
			// 			DATE_FORMAT(kapel.signdt, '%d-%m-%Y %H:%i:%s') as app_pel, 
			...
			// 			) ka  ON k.nomorkontrak = ka.nmrkontrak LEFT JOIN 
			...
			// 				GROUP BY nomorkontrak
			// 			) kapel  ON k.nomorkontrak = kapel.nomorkontrak LEFT JOIN
			// 			(
			// 				SELECT	nomorkontrak, max(signdt) as signdt
			// 				FROM	kontrak_approval
			// 				Where	signlevel = 3 and actiontype = 1
			// 				GROUP BY nomorkontrak
			// 			) kaang  ON k.nomorkontrak = kaang.nomorkontrak LEFT JOIN
			// 			(
			// 				SELECT	nomorkontrak, max(signdt) as signdt
			// 				FROM	kontrak_approval
			// 				Where	signlevel = 4 and actiontype = 1
			// 				GROUP BY nomorkontrak
			// 			) kakeu  ON k.nomorkontrak = kakeu.nomorkontrak
			...
			$sql = "
				SELECT 	n.nomornota, pelaksana, b.namaunit, pos1, nilai1, namapos, nomorwbs, nomorprk, 
						nomorscore,	nomorskki noskk, s.uraian uraians, s.nilaianggaransl1, s.nilaianggarankp, 
						DATE_FORMAT(tanggalskki, '%d-%m-%Y') as tanggalskki, nilaianggaran anggaran, s.jtm, ka.*, 
						nilaidisburse disburse, nilaiwbs wbs, pdp.file_path as pdp_file_path, s.sl1, rb.no_rab, 
						DATE_FORMAT(inputdt, '%d-%m-%Y %H:%i:%s') as inputdt, k.nomorkontrak, vendor, k.kid, 
			...
			while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
				...
				if($row["kontrakapproved"] == 1){
					...
					if(($row["signlevel"] == 0 && $row["actiontype"] == 1) || ($row["signlevel"] > 1 && $row["actiontype"] == 0)){
						...
						$statusbayar = "Kontrak sudah di inbox bayar UP3.";
				...
				if($dummy != $row["noskk"] || $dummypos != $row["pos1"]) {
					if($no>0) {
						...
						$hasilp .= "
								...
								<td align='right'>".number_format($pjtmaset, 2, '.', ',')."</td>
								<td align='right'>".number_format($pjtmrp)."</td>
								<td align='right'>".number_format($pgdaset, 2, '.', ',')."</td>
								<td align='right'>".number_format($pgdrp)."</td>
								<td align='right'>".number_format($pjtraset, 2, '.', ',')."</td>
								<td align='right'>".number_format($pjtrrp)."</td>
								<td align='right'>".number_format($psl1aset, 2, '.', ',')."</td>
								<td align='right'>".number_format($psl1rp)."</td>
								<td align='right'>".number_format($psl3aset, 2, '.', ',')."</td>
								<td align='right'>".number_format($psl3rp)."</td>
								<td align='right'>".number_format($pkeypointaset, 2, '.', ',')."</td>
				...
				if($dummy != $row["noskk"]) {
					if($no>0) {
						$hasils .= "
								...
								<td align='right'>".number_format($sjtmaset, 2, '.', ',')."</td>
								<td align='right'>".number_format($sjtmrp)."</td>
								<td align='right'>".number_format($sgdaset, 2, '.', ',')."</td>
								<td align='right'>".number_format($sgdrp)."</td>
								<td align='right'>".number_format($sjtraset, 2, '.', ',')."</td>
								<td align='right'>".number_format($sjtrrp)."</td>
								<td align='right'>".number_format($ssl1aset, 2, '.', ',')."</td>
								<td align='right'>".number_format($ssl1rp)."</td>
								<td align='right'>".number_format($ssl3aset, 2, '.', ',')."</td>
								<td align='right'>".number_format($ssl3rp)."</td>
								<td align='right'>".number_format($skeypointaset, 2, '.', ',')."</td>
...
*/
				...
				$hasilk .= "
					<tr>
						...
						<td align='right'>".number_format($row["jtmaset"], 2, '.', ',')."</td>
						<td align='right'>".number_format($row["jtmrp"])."</td>
						<td align='right'>".number_format($row["gdaset"], 2, '.', ',')."</td>
						<td align='right'>".number_format($row["gdrp"])."</td>
						<td align='right'>".number_format($row["jtraset"], 2, '.', ',')."</td>
						<td align='right'>".number_format($row["jtrrp"])."</td>
						<td align='right'>".number_format($row["sl1aset"], 2, '.', ',')."</td>
						<td align='right'>".number_format($row["sl1rp"])."</td>
						<td align='right'>".number_format($row["sl3aset"], 2, '.', ',')."</td>
						<td align='right'>".number_format($row["sl3rp"])."</td>
						<td align='right'>".number_format($row["keypointaset"], 2, '.', ',')."</td>
			...
			echo "
				<tfoot>
					<tr>
						...
						<td></td>
						<td align='right'>" . number_format($ang_jtm_assett, 2, '.', ',') . "</td>
						<td align='right'>" . number_format($ang_jtm_rpt) . "</td>
						<td align='right'>" . number_format($ang_gd_assett, 2, '.', ',') . "</td>
						<td align='right'>" . number_format($ang_gd_rpt) . "</td>
						<td align='right'>" . number_format($ang_jtr_assett, 2, '.', ',') . "</td>
						<td align='right'>" . number_format($ang_jtr_rpt) . "</td>
						<td align='right'>" . number_format($ang_sl1_assett, 2, '.', ',') . "</td>
						<td align='right'>" . number_format($ang_sl1_rpt) . "</td>
						<td align='right'>" . number_format($ang_sl3_assett, 2, '.', ',') . "</td>
						<td align='right'>" . number_format($ang_sl3_rpt) . "</td>
						<td align='right'>" . number_format($ang_kp_assett, 2, '.', ',') . "</td>
						...
						<td align='right'>" . number_format($rea_jtm_assett, 2, '.', ',') . "</td>
						<td align='right'>" . number_format($rea_jtm_rpt) . "</td>
						<td align='right'>" . number_format($rea_gd_assett, 2, '.', ',') . "</td>
						<td align='right'>" . number_format($rea_gd_rpt) . "</td>
						<td align='right'>" . number_format($rea_jtr_assett, 2, '.', ',') . "</td>
						<td align='right'>" . number_format($rea_jtr_rpt) . "</td>
						<td align='right'>" . number_format($rea_sl1_assett, 2, '.', ',') . "</td>
						<td align='right'>" . number_format($rea_sl1_rpt) . "</td>
						<td align='right'>" . number_format($rea_sl3_assett, 2, '.', ',') . "</td>
						<td align='right'>" . number_format($rea_sl3_rpt) . "</td>
						<td align='right'>" . number_format($rea_kp_assett, 2, '.', ',') . "</td>
