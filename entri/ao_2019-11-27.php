		...
		if($v!="") {
			$sql = "
				SELECT	n.nomornota, b.namaunit, pos1, nilai1, namapos, nomorwbs, vendor, nomorcostcenter, 
						...
						nilaikontrak kontrak, k.uraian uraiank, bayar, k.kid, k.signed, kt.nama as name_type, ka.*,
						DATE_FORMAT(inputdt, '%d-%m-%Y %H:%i:%s') as inputdt, 
						DATE_FORMAT(tglawal, '%d-%m-%Y') as tglawal, DATE_FORMAT(tglakhir, '%d-%m-%Y') as tglakhir,
						DATE_FORMAT(date(k.tgltagih), '%m-%Y') as tgltagih, 
						DATE_FORMAT(kapel.signdt, '%d-%m-%Y %H:%i:%s') as app_pel, 
						DATE_FORMAT(kaang.signdt, '%d-%m-%Y %H:%i:%s') as app_ang, 
				...
				FROM 	notadinas n LEFT JOIN 
						...
						(
							...
							GROUP BY nomorkontrak
						) kakeu  ON k.nomorkontrak = kakeu.nomorkontrak LEFT JOIN
