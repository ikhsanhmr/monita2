...
//	$parm .= ($p2==""? "": " and SUBSTR(tanggalskki, 1, 7) <= '$p2'");
	...
	$parm .= ($p1==""? "": " and tanggalskki >= '" . substr($p1,0,4) . "-" . substr($p1,-2) . "-01'");
	$parm .= ($p2==""? "": " and tanggalskki <= '" . substr($p2,0,4) . "-" . substr($p2,-2) . "-31'");
...
//		$parm .= ($kdpos0==""? "": " and pos1 = '$kdpos0'");
	...
	if($v!="") {
		...
		$sql = "
			SELECT	b.namaunit, nomorscore, nomorwbs, nomorprk, nomorskki noskk, s.uraian uraians, 
					DATE_FORMAT(tanggalskki, '%d-%m-%Y') as tanggalskki, DATEDIFF(SYSDATE(), tanggalskki) umur,
					nilaianggaran anggaran, nilaidisburse disburse, nilaiwbs wbs, COALESCE(kontrak,0) kontrak, 
					COALESCE(bayar,0) bayar, s.gd gd, s.sl3 sl3, s.nilaianggarangd nilaianggarangd, s.jtm jtm, 
					s.nilaianggaranjtm nilaianggaranjtm, s.jtr jtr, s.nilaianggaranjtr nilaianggaranjtr, 
					s.sl1 sl1, s.keypoint keypoint, s.nilaianggaransl1 nilaianggaransl1, 
					s.nilaianggaransl3 nilaianggaransl3, s.nilaianggarankp nilaianggarankp
