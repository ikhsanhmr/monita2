	...
	if($v!="") {
		$sql = "
			SELECT	n.nomornota, nipuser, pelaksana, b.namaunit, pos1, nilai1, namapos, nomorwbs, nomorcostcenter, 
					nomorskko noskk, s.uraian uraians, DATE_FORMAT(tanggalskko, '%d-%m-%Y') as tanggalskko, 
					nilaianggaran anggaran, nilaidisburse disburse, nilaiwbs wbs, nilaikontrak kontrak,
					DATE_FORMAT(inputdt, '%d-%m-%Y %H:%i:%s') as inputdt, k.nomorkontrak, vendor, k.uraian uraiank, 
					bayar, k.kid, k.signed, k.file_path, DATE_FORMAT(kaang.signdt, '%d-%m-%Y %H:%i:%s') as app_ang, 
					DATE_FORMAT(tglawal, '%d-%m-%Y') as tglawal, DATE_FORMAT(tglakhir, '%d-%m-%Y') as tglakhir, ka.*, 
					kt.nama as name_type, DATE_FORMAT(date(k.tgltagih), '%m-%Y') as tgltagih, 
			...
			FROM 	notadinas n LEFT JOIN 
					...
					) kakeu  ON TRIM(k.nomorkontrak) = TRIM(kakeu.nmrkontrak) LEFT JOIN
