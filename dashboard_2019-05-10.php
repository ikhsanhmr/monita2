...
if($_SESSION['roleid']<=3 || $_SESSION['roleid'] == 20) {
	...
	$sql = "SELECT 	COUNT(k.nomorkontrak) jumlah 
			...
			Where " . ( $_SESSION['roleid'] > 1 ? "((pelaksana <= 5 and (d.pos1 IN (Select akses From akses_pos Where nip = '".$_SESSION["cnip"]."'))) OR (pelaksana > 5 and (pelaksana in (Select akses From akses_bidang Where nip = '".$_SESSION['cnip']."') $sisip))) AND " : "" ) . " (signlevel = 2 AND actiontype = 1)";
	//echo "$sql<br>";
	$result = mysql_query($sql) or die (mysql_error());
	while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
		$ib = $row["jumlah"];
