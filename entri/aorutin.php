...
<body>
	...
	$parm .= ($b0 == "" ? "" : " and (nipuser = '$b0' or c.nick = '$b0')");
	...
	if ($v != "") {
		...
		// 				SUM(nilai_des) AS nilai_des, SUM(nilai_utang) AS nilai_utang, 
		// 				SUM(bayar_jan) AS bayar_jan, SUM(bayar_feb) AS bayar_feb, SUM(bayar_mar) AS bayar_mar, 
		// 				SUM(bayar_apr) AS bayar_apr, SUM(bayar_mei) AS bayar_mei, SUM(bayar_jun) AS bayar_jun, 
		// 				SUM(bayar_jul) AS bayar_jul, SUM(bayar_ags) AS bayar_ags, SUM(bayar_sep) AS bayar_sep, 
		// 				SUM(bayar_okt) AS bayar_okt, SUM(bayar_nov) AS bayar_nov, SUM(bayar_des) AS bayar_des, 
		...
		// 					ORDER BY a.id, a.nama
		// 				) AS md
		// 				GROUP BY id, nama";

		$sql = "
				SELECT	k.id, k.nama, nilai_jan, nilai_feb, nilai_mar, nilai_apr, nilai_mei, nilai_jun, 
						nilai_jul, nilai_ags, nilai_sep, nilai_okt, nilai_nov, nilai_des, nilai_utang, bayar_jan, 
						bayar_feb, bayar_mar, bayar_apr, bayar_mei, bayar_jun, bayar_jul, bayar_ags, bayar_sep, 
						bayar_okt, bayar_nov, bayar_des, bayar_utang
				FROM	(
							SELECT	id, nama, SUM(nilai_jan) AS nilai_jan, SUM(nilai_feb) AS nilai_feb,
							...
							FROM 	(
										...
										FROM	kontrak_type kt INNER JOIN 
												(
													...
													WHERE	YEAR(d.tanggalskko) = $p1 and YEAR(e.tgltagih) IN (" . ($p1 - 1) . ", $p1) $parm
