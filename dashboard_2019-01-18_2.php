...
if($_SESSION['roleid']<=3) {
	...
	$sql = "SELECT 	COUNT(*) jumlah 
			...
			Where k.pos IN (Select akses From akses_pos Where nip = '".$_SESSION['cnip']."') AND (signlevel = 2 AND actiontype = 1)";
	//echo "$sql<br>";
	$result = mysql_query($sql) or die (mysql_error());
	while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
		$ib = $row["jumlah"];
	...
	$sql = "SELECT 	COUNT(*) jumlah 
			FROM 	notadinas_detail nd INNER JOIN 
					kontrak k ON nd.noskk = k.nomorskkoi INNER JOIN 
					( 
						SELECT 	t1.nomorkontrak nmrkontrak, signed, signdt, signlevel, actiontype 
						FROM 	kontrak_approval t1 WHERE t1.id = (
																	SELECT 	t2.id 
			...
			Where pelaksana IN (Select akses From akses_bidang Where nip = '".$_SESSION['cnip']."') AND (signlevel = 3 AND actiontype = 1)";
	//echo "$sql<br>";
	$result = mysql_query($sql) or die (mysql_error());
	while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
		$ib = $row["jumlah"];
	...
	$sql = "Select Count(*) jumlah 
			FROM (
					...
					Where pelaksana = ".$_SESSION['bidang']." AND (signlevel = 1 AND actiontype = 1) Group By nomorkontrak
			) as kontrak";
	//echo "$sql<br>";
	$result = mysql_query($sql) or die (mysql_error());
	while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
...
// $sql = "SELECT sum(nilaidisburse) as skkimurni, sum(nilaikontrak) as realisasi
		// FROM (
			...
			// WHERE 	YEAR(a.tanggalskki) = ".date("Y")." AND b.pos IN ('62.2','62.3','62.4','62.5','62.6','62.7','62.8','62.9','62.10')
...
// $sql = "SELECT sum(nilaidisburse) as skkimurni, sum(nilaikontrak) as realisasi
		// FROM (
			...
			// WHERE 	YEAR(a.tanggalskki) = ".date("Y")." AND b.pos IN ('62.2','62.3','62.4','62.5','62.6','62.7','62.8','62.9','62.10') AND b.no_rab = ''
			// Group By b.nomorskkoi, a.nilaidisburse
		// ) as data";
...
$skaimurnikontrak = $row2['disburse'];
...
/*$sql = "SELECT	d.tahun, e.akipos AS nilaiaki, SUM( d.nilaibayar ) AS realisasi
		FROM (
				...
				WHERE YEAR( a.tanggalskki ) = ".date("Y")." AND b.pos IN ('62.1','62.2','62.3','62.4','62.5','62.6','62.7','62.8','62.9','62.10')
...
// $sql = "SELECT unit.id, unit.namaunit, IFNULL(e.rpaki,0) AS nilaiaki, SUM(IFNULL(d.nilaibayar, 0)) AS realisasi
		...
		// GROUP BY unit.id, unit.namaunits
				...
				(
					...
					FROM	notadinas_detail e LEFT JOIN 
							(
								...
								WHERE YEAR( tanggalskki ) = ".date("Y")." AND pos IN ('62.1','62.2','62.3','62.4','62.5','62.6','62.7','62.8','62.9','62.10')
...
while ($row = mysql_fetch_assoc($result)) {
	...
	if ($skaimurnikontrak > 0){
		$kontrakvsmurnibidang = round(($realisasikontrak / $skaimurnikontrak) * 100, 2);
	}
