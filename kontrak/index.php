...
*/		
		...
		echo "
			...
			".($_SESSION["roleid"] >= 1 && $_SESSION["roleid"] <= 3 ? "<a href='#' onClick='upload()'>(+) Upload Kontrak</a><br />" : "").
			...
			while ($row = mysqli_fetch_array($result)) {
				...
				$parm .= "
					<tr>
						...
						//($row["skkoi"]=="SKKI"? 
							($row["sgd"]==""? "<a href='#' onClick='edit(\"$row[kid]\")'>Edit</a><br><a href='#' onClick='hapus(\"".str_replace("'","",$row["nomorkontrak"])."\", \"$row[kid]\")'>Hapus</a>": "<strong>SIGNED</strong>")//: 
