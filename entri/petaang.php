...
<?php
	...
	$sql = "
	SELECT v.*, rppos, terbit, kontrak, bayar 
	FROM 	(
				SELECT DISTINCT	akses, nama 
				FROM 	v_pos " . ($org>5 && $org<16? "":  ($org>="1"? " where nip='$nip'": "") ) . "
				UNION
				...
				" . (($org>5 && $org<16) ? "":  (($org>="1") ? " where nip='$nip' $parm": "") ) . " 
				ORDER BY akses
			) v LEFT JOIN 
			(
				SELECT kdsubpos, rppos FROM saldopos WHERE tahun = $y
			...
			(
				SELECT 	pos1, SUM(nilai1) terbit 
				FROM 	notadinas_detail d LEFT JOIN 
						notadinas n ON d.nomornota = n.nomornota 
				WHERE YEAR(tanggal) = $y AND d.progress >= 7 GROUP BY pos1
			) d ON v.akses = d.pos1	LEFT JOIN 
			(
				...
				FROM 	(
							...
							FROM 	notadinas_detail d LEFT JOIN 
									notadinas n ON d.nomornota = n.nomornota 
							WHERE YEAR(tanggal) = $y AND d.progress >= 7 
							-- GROUP BY pos1
						) nd LEFT JOIN 
			...
			(
				...
				WHERE NOT nokontrak IS NULL	GROUP BY pos 
			) b ON v.akses = b.pos
	order by akses";
